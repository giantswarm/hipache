...
Etcd.prototype.getAll = function(cb) {
    this.client.get(this.config.etcd.path, { recursive: true }, function(err, data) {
        ...
        var backends = {};
        if (data.node.nodes) {
            data.node.nodes.forEach(function(domainNode) {
                if (domainNode.nodes) {
                    domainNode.nodes.forEach(function(backendNode) {
                        var backend  = this.getHostnameFromKey(backendNode.key);

                        if (!backends[backend.hostname]) {
                            backends[backend.hostname] = [];
                        }

                        backends[backend.hostname].push({
                            backend: backend,
                            url: backendNode.value
                        });
...
Etcd.prototype.updateAllBackends = function(backends) {
    this.redisClient.flushdb(function(err) {
        Object.keys(backends).forEach(function(hostname) {
            if (backends[hostname].length > 0) {
                this.initBackend(backends[hostname][0].backend, function() {

                    backends[hostname].forEach(function(backend) {
                        this.addBackend(backend.backend, backend.url);
                    }.bind(this));

                }.bind(this));
            } else {
                console.error("No backend found for hostname: ", hostname);
            }
        }.bind(this));
    }.bind(this));
};
...
Etcd.prototype.watch = function() {
    ...
    watcher.on("resync", console.info);
    watcher.on("reconnect", console.info);
